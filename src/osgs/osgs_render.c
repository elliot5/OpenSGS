...
static int node_editor(struct nk_context *ctx, waveprp_t *wave_properties)
{
    int n = 0;
    ...
    if (nk_begin(ctx, "NodeEdit", nk_rect(100, 100, 1000, 650),
        NK_WINDOW_BORDER|NK_WINDOW_NO_SCROLLBAR|NK_WINDOW_MOVABLE|NK_WINDOW_TITLE))
    {
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodedit->node_count);
        {
            ...
            while (it) {
                ...
                {
                    /* node context menu */
                    if(contextual_opened == 0) {
                        if (nk_contextual_begin(ctx, 0, nk_vec2(100, 220), it->bounds)) {
                            nk_layout_row_dynamic(ctx, 25, 1);
